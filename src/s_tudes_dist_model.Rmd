...
benthic Current velocity range, light at bottom range, benthic silicate mean, surface current velocity range, surface iron range, benthic chlorophil range, benthic chlorophil mean, surface dissolved oxygen range
...
write.table(data.frame(bioclim = bioclim.e, gower = gower.e, maha = maha.e,maxent = maxent.e),
            here::here("data", "outputs", "AUC_present.txt"), sep = "\t", row.names = F)
write.table(data.frame(bioclim = bioclim.t, gower = gower.t, maha = maha.t,maxent = maxent.t),
            here::here("data", "outputs", "Threshold_present.txt"), sep = "\t", row.names = F)
```
...
auc <- read.table(here::here("data", "outputs", "AUC_present.txt"), h = T)
...
```{r}
# Tif 1
writeRaster(x = gensemble[[1]], filename = file.path(here::here("data", "outputs", "ensemble_stud_binary_present.tif"),
                                                     sep=""),
            format = 'GTiff', overwrite=T)
plot(ensemble)
writeRaster(x = ensemble, filename = file.path(here::here("data", "outputs", "ensemble_stud_prob_present.tif"),
...
```{r}
# Binary map
stud_bin_pres <- raster::raster(here::here("data", "outputs", "ensemble_stud_binary_present.tif"))
# Probabilty map
stud_prob_pres <- raster::raster(here::here("data", "outputs", "ensemble_stud_prob_present.tif"))
...
```{r}
ggplot() +
  geom_point(data = stud_bin_pres_df, 
            aes(x = x, y = y)) +
...
```{r}
ggplot() +
  geom_point(data = stud_prob_pres_df, 
            aes(x = x, y = y)) +
  scale_color_viridis_b() +
    geom_point(data = ocor, 
             aes(x = long, y = lat),
