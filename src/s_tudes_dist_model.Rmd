...
```{r pdf file test}
suri_pdf <- extract_tables(here::here("data", "project_global_country_profile_suriname_draft.pdf"), pages = 16)
...
```{r}
env_layers <- list.files(file.path(presdir, "used"), pattern =".tif", full.names = TRUE)
```

```{r}
#read in the data in .txt
ocor <- read.table(file.path(occurdir, "occurrence_data.txt"), h = T)
```
...
# Checking for the range values of each environmental layer

```{r}
#seleciona o plot para uma determinada variavel
plot(predictors[[1]]) # Benthic mean dissolved oxygen
map(add=T)
points(ocor, pch=20) 


plot(predictors[[2]]) # Benthic mean light at bottom
map(add=T)
points(ocor, pch=20) 

plot(predictors[[3]]) # Benthic mean salinity
map(add=T)
points(ocor, pch=20) 


plot(predictors[[4]]) # Surface mean chlorophyll
map(add=T)
points(ocor, pch=20) 

plot(predictors[[5]]) # Surface mean cloud cover
map(add=T) # This is the only layer with values between 0 and 1
points(ocor, pch=20) 

plot(predictors[[6]]) # Surface mean current velocity
map(add=T)
points(ocor, pch=20) 

plot(predictors[[7]]) # Surface mean dissolved oxygen
map(add=T)
points(ocor, pch=20) 

plot(predictors[[8]]) # Surface mean salinity
map(add=T)
points(ocor, pch=20) 

plot(predictors[[9]]) # Surface mean temperature
map(add=T)
points(ocor, pch=20) 

plot(predictors[[10]]) # Surface temperature range
map(add=T)
points(ocor, pch=20) 
```
...
variables3 <- vifcor(variables, th = 0.1)
variables3
# Variable removed after collinearity analysis:
...
#write_csv(correlation_df, file.path(presdir, "env_layers_correlation.csv"), col_names = T)
```

- Variables with high correlation based on the cor() function: Benthic current velocity mean and range, benthic light at bottom mean and range, benthic primary productivity mean and range, benthic temperature mean and surface temperature mean, benthic temperature range and surface temperature range, cloud cover and surface salinity range, surface dissolved oxygen range and surface salinity range, and surface primary productivity mean and range.

Variables chosen:
- Benthic mean current velocity
...
- Surface dissolved oxygen mean
- surface primary productivity mean
- Surface pH


- Variables removed after vifstep() function:
- Surface dissolved oxygen mean
- Benthic salinity range
- Surface temperature mean


##### FINAL LIST OF VARIABLES EMPLOYED
- Benthic mean current velocity
- Benthic light at bottom range
- Benthic primary productivity mean
- Benthic dissolved oxygen mean
...
write.table(data.frame(bioclim = bioclim.e, gower = gower.e, maha = maha.e,maxent = maxent.e),
            file.path(outdir, "AUC_complete2.txt"), sep = "\t", row.names = F)
write.table(data.frame(bioclim = bioclim.t, gower = gower.t, maha = maha.t,maxent = maxent.t),
            file.path(outdir, "Thresh_complete2.txt"), sep = "\t", row.names = F)
```
...
auc <- read.table(file.path(outdir, "AUC_complete2.txt"), h = T)
...
writeRaster(x = gensemble[[1]], 
            filename = file.path(outdir, "ensemble_stud_binary_present_comparison2.tif"), sep="",
            format = 'GTiff', overwrite=T)
plot(ensemble)
writeRaster(x = ensemble, 
            filename = file.path(outdir, "ensemble_stud_prob_present_comparison2.tif"), sep="",
            format = 'GTiff', overwrite=T)

tiff(file="ensembled_binary_pres_comparison2.tiff", width = 3200, height = 3200, res = 400, compression = "lzw")
plot(gensemble[[1]])
points(ocor, pch=20) 
map(add=T)
dev.off()

tiff(file="ensembled_prob_pres_comparison2.tiff", width = 3200, height = 3200, res = 400, compression = "lzw")
plot(ensemble)
...
tiff(file="ensemble_prob_smaller_pres_comparison2.tiff", width = 3200, height = 3200, res = 400, compression = "lzw")
plot(fig.menor)
points(ocor, pch=20) #inserir pontos de ocorr?ncia no mapa
map(add=T)
dev.off()
