...
#GBIF
stud_data_gbif <- read.csv(here("data","occurrences", "s_tudes_occurrences_raw_gbif.csv"), sep = ";") %>% 
  clean_names() %>% 
...
#OBIS
stud_data_obis <- read.csv(here("data","occurrences", "obis_raw.csv"), sep = ";") %>% 
  clean_names() %>% 
...
#Fish net
stud_data_fishnet <- read.csv(here("data","occurrences", "fish_net_2_raw.csv"), sep = ";") %>% 
  clean_names() %>% 
...
list_layers(c("Bio-ORACLE", monthly = F))$layer_code

# Load selected layers based on knowledge about the species biology
env_layers <- load_layers(c("BO2_salinitymean_bdmean", "BO2_chlomean_bdmean", "BO_bathymax", "BO_bathymin", 
                       "BO2_dissoxmean_bdmean", "BO2_ironmean_bdmean", "BO2_lightbotmean_bdmean", 
                       "BO2_tempmean_bdmean", "BO2_ppmean_bdmean"))
```
...
ocor.val <- extract(predictors, gcir_ocur, cell = T)
...
gcir_ocur <- cbind(gcir_ocur, ocor.val == FALSE)

gcir_ocur <- gcir_ocur[which(id.dup == FALSE),]

gcir_ocur <- na.omit(gcir_ocur)
...
variables <- gcir_ocor[,3:8]

correlation <- (cor(variables)> 0.7)*1

write.csv2(correlation, "correlation.csv")
```
...
```{r}
bg <- randomPoints(predictors, nrow(gcir_ocur), ext = map_extent)
...
data <- prepareData(x = predictors, p = gcir_ocur[, 1:2], b = back[, 1:2])
```
...
```{r}
for(i in 1:20){
  id.treino<-sample(1:nrow(gcir_ocur),round(0.75*nrow(gcir_ocur),0))  ###0.75 ? pra pegar 75% dos pontos de ocorrencia pra treino, o round ? s? pra nao dar numero quebrado
  treino<-prepareData(x = predictors, p = gcir_ocur[id.treino,1:2], b = back[id.treino,1:2])  ##o id.treino ta falando que ? pra pegar s? as linhas do id.treino
  teste<-prepareData(x = predictors, p = gcir_ocur[-id.treino,1:2], b = back[-id.treino,1:2])  ##aqui o -id.treino ? pra tirar os usados no id.treino
...
```{r}
# FIGURE OUT HOW NOT TO USE THIS AND SEND RESULTS TO THE OUTPUTS FOLDER
setwd("~/BACKUP NOTEBOOK/DOCUMENTS/Leonardo/Mestrado UFPE/Projeto Mestrado UFPE/Projeto C. porosus/Cap?tulo 1/Mestrado - Leo/outputsleo")
...
```{r}
# Tif 1
writeRaster(x = gensemble[[1]], filename = paste(" (ensemble.05.1).tif",sep=""), format = 'GTiff', overwrite=T)
plot(ensemble)
writeRaster(x = ensemble, filename = paste(" (ensemble2.05.1).tif", sep=""), format = 'GTiff', overwrite=T)

tiff(file="ensembled.05NCSA.2.tiff", width = 3200, height = 3200, res = 400, compression = "lzw")
plot(gensemble[[1]])
points(ocor, pch=20) #inserir pontos de ocorr?ncia no mapa
map(add=T)
dev.off()

tiff(file="ensembled2.1.05NCSA.2.tiff", width = 3200, height = 3200, res = 400, compression = "lzw")
plot(ensemble)
points(ocor, pch=20) #inserir pontos de ocorr?ncia no mapa
map(add=T)
dev.off()
...
tiff(file="fig.menor2.tiff", width = 3200, height = 3200, res = 400, compression = "lzw")
plot(fig.menor)
points(ocor, pch=20) #inserir pontos de ocorr?ncia no mapa
map(add=T)
dev.off()
